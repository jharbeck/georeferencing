...
;	worker_output_dir: location to place worker output files
;EXAMPLE
;	kk=10
...
;Updated by Jeremy Harbeck, 9/5/2018: Version 2 - made windows/linux compliant
;-

pro split_for_v3, start, finish, varnames=varnames, nsplit=nsplit, $
               outvar=outvar, commands=commands, $
               ctvariable_name=ctvariable_name, struct2pass1=struct2pass1,$
               struct2pass2=struct2pass2, struct2pass3=struct2pass3,$
               struct2pass4=struct2pass4, struct2pass5=struct2pass5, $
	       ...
	       levoff=levoff, halt=halt, wait_interval=wait_interval,worker_output_dir=worker_output_dir
;if ~keyword_set(levoff) then if scope_level() EQ 2 then levoff=2

slash = path_sep()
if n_elements(worker_output_dir) eq 0 then worker_output_dir = ''
caldat,systime(/julian,/utc),month,day,year,hour,minute,second
out_timestamp = timestamp(year=year,month=month,day=day)+'T'+strcompress(add_zeroes2(hour,2)+'-'+add_zeroes2(minute,2)+'-'+add_zeroes2(round(second),2),/remove_all)+'Z'
...
if n_elements(debug) NE 0 AND x_chkfil('simple_batch_451451', /silent) EQ 0 then begin
    ...
    split_for_v3, start, finish, varnames=varnames, nsplit=nsplit, $
      outvar=outvar, commands=commands, $
      ctvariable_name=ctvariable_name, struct2pass1=struct2pass1,$
      struct2pass2=struct2pass2, struct2pass3=struct2pass3,$
      struct2pass4=struct2pass4, struct2pass5=struct2pass5, $
      ...
      struct2out2=struct2out2, struct2out3=struct2out3,$
      struct2out4=struct2out4, struct2out5=struct2out5, test=test, $
      after_loop_commands=after_loop_commands, debug=debug, levoff=l1, halt=halt, test=test,out_file_dir=out_file_dir
endif else begin
    ...
    if n_elements(debug) EQ 0 then begin
        if strcmp(worker_output_dir,'') eq 1 then output_file='' else output_file = worker_output_dir+'Run_'+out_timestamp+'_worker0_output.txt'
        obridge=obj_new("IDL_IDLBridge", output=output_file)

        if nsplit GT 1 then begin
            for i=1, nsplit-1 do begin
                if strcmp(worker_output_dir,'') eq 1 then output_file = '' else output_file = worker_output_dir+'Run_'+out_timestamp+'_worker'+strcompress(i,/remove_all)+'_output.txt'
                obridge=[obridge, obj_new("IDL_IDLBridge",output=output_file)]
            endfor
        endif

        if strcmp(slash,'/') eq 1 then spawn, 'pwd', pwd else spawn, 'cd', pwd
        fluffx123123pwd=pwd

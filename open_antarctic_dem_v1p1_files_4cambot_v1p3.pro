Function open_antarctic_dem_v1p1_files_4cambot_v1p3,center_lat,center_lon,center_elev,diag_fov=diag_fov,update=update,xygrids=xygrids
;--------------------------------------------------------------------------------
...
;Version 1p3 : antarctic version starts here...
;
;Keywords
...
dem_base_dir = base_dir4+sl+'rema_v1p1'
indexfile = dem_base_dir+sl+'REMA_Tile_Index_Rel1_1'+sl+'REMA_Tile_Index_Rel1_1.shp'
if file_test(indexfile) eq 0 then STOP,'cannot find index file'
...
FOR b=0,num_ent-1 do begin
  ...
  tile_name[b] = attr[b].attribute_0 ;assign tile name: attribute_1 in ArcticDEM, attribute_0 in REMA
  tile_url[b] = attr[b].attribute_6 ;attribute_7 in ArcticDEM, attribute_6 in REMA
...
;names and url are already setup to work with 8m REMA tiles
tile_name_8m = tile_name ;strmid(tile_name,0,6)+'8'+strmid(tile_name,11)
tile_url_8m =  tile_url ;strmid(tile_url,0,61)+'8'+strmid(tile_url,62,14)+'8'+strmid(tile_url,81) ;files are downloaded from the PGC site, which is relative to the WGS-84 ellipsoid

;save,filename=sav_filename,x_vert,y_vert,tile_name,tile_url,tile_name_8m,tile_url_8m
...
diag_dist = 2000 ;(center_elev*tan(diag_fov*(!pi/180.0)))[0]+extra_dist
...
;                     8m resolution
;-----------------------------------------------------
point_tile_url_temp = tile_url_8m[point_tile_sel2]
point_tile_name_temp = tile_name_8m[point_tile_sel2]

point_tile_sel_8m = uniq(point_tile_name_temp,sort(point_tile_name_temp))
n_tiles = n_elements(point_tile_sel_8m)
point_tile_name = point_tile_name_temp[point_tile_sel_8m]
point_tile_url = point_tile_url_temp[point_tile_sel_8m]
...
for f=0,n_tiles-1 do begin
  ...
  tif_filename = dem_base_dir+sl+point_tile_name[f]+'_dem.tif'
  ...
  if file_test(tif_filename) eq 0 then begin
...
dem_tile_size = [12500,12500] ;all REMA tiles are 12500x12500
...
for t=0,n_tiles-1 do begin
  ...
  shp_filename = dem_base_dir+sl+'index'+sl+point_tile_name[t]+'_index.shp'
  tif_filename = dem_base_dir+sl+point_tile_name[t]+'_dem.tif'
  dem = read_arctic_dem_basic_v1p1(shp_filename,tif_filename,min_x,max_x,min_y,max_y)  ;REMA & ArcticDEM data is available only as a 32-bit datatype i.e. a float.
  ;returns: 
  ;elev
  ;x_axis
  ;y_axis
